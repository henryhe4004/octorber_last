        ...
        GeometryReader { geometry in
            ScrollView(.vertical, showsIndicators: false){
                ...
                VStack{
                    ...
                    HStack{
                        Text("\(liner.eventContent)").frame( alignment: .topLeading)
//                            .offset(x: 30, y: 0)
                            .padding(EdgeInsets(top: 10, leading: 40, bottom: 0, trailing: 40))
                    }
                    HStack{
                    LazyVGrid(columns: items, content: {
                        ForEach(0..<liner.img.count,id: \.self){
                            index in
                            KFImage.url(URL(string:liner.img[index]))
...
//                        .lineSpacing(12.0)
                    HStack{
                        Spacer()
                        Text("上传时间：\(liner.eventTime)").foregroundColor(grayColor).padding(EdgeInsets(top: 5, leading: 0, bottom: 10, trailing: 15))
                            .font(.system(size: 15))
                    }
